...
def cached_analysis(kind, analysis_date, context, make_analysis):
    ...
    if cached:
        ...
        if cached[0]["input_hash"] == digest or still_fresh:
            return cached[0]["content"], False
    content = make_analysis()
    db.table("analysis_cache").upsert({"kind":kind, "analysis_date":str(analysis_date), "input_hash":digest, "content":content}, on_conflict="kind,analysis_date").execute()
    return content, True
    ...
    if week_meals or month_meals_for_advice:
        try:
            ...
            with st.spinner("栄養傾向を確認中..."):
                week_advice, _ = cached_analysis("antiaging_week", week_scope, week_context, lambda: nutrition_coach(week_context, goal, diet_notes)) if week_meals else (None, False)
                month_advice, _ = cached_analysis("antiaging_month", month_scope, month_context, lambda: nutrition_coach(month_context, goal, diet_notes)) if month_meals_for_advice else (None, False)
            if week_advice:
                st.markdown("**直近7日の傾向**")
                st.info(week_advice)
            if month_advice:
                st.markdown("**直近30日の傾向**")
                st.info(month_advice)
        except Exception as exc:
