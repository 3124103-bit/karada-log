...
相談: {question}"""
    return client.responses.create(model=secret("OPENAI_MODEL") or "gpt-4.1-mini", input=prompt).output_text
...
次の形式で、短く具体的に日本語で答えてください。
1. 今日の不足しやすい栄養素（最大3つ、断定せず理由も一言）
2. 次の一食の提案（料理の組み合わせ）
3. 飲み物・間食の提案
4. サプリ候補（食事で補えない場合のみ。服薬・持病・妊娠中は医師または薬剤師に確認と必ず明記）
5. 今日の控えめにしたいもの
記録: {json.dumps(context, ensure_ascii=False, default=str)}"""
    return OpenAI(api_key=secret("OPENAI_API_KEY")).responses.create(
        model=secret("OPENAI_MODEL") or "gpt-4.1-mini", input=prompt
    ).output_text
...
with tabs[5]:
    ...
    if goal in ["アンチエイジング＋脂肪を落とす", "脂肪を落とす"]:
        calorie_target = max(bmr * 1.25, bmr * 1.4 + cardio_burned + strength_bonus - 300)
        protein_target, fat_target = weight_kg * 1.8, weight_kg * 0.8
    elif goal == "筋肉を増やす":
    ...
    st.caption(f"目標：{goal}　/　基礎代謝の基準：{bmr:.0f} kcal　/　有酸素消費：{cardio_burned:.0f} kcal　/　筋トレ：{len(strength)} 件")
    targets = pd.DataFrame([
        {"栄養素":"カロリー", "目安":f"{calorie_target:.0f} kcal", "現在":f"{total('calories'):.0f} kcal", "差分":f"{calorie_target-total('calories'):+.0f} kcal"},
        {"栄養素":"たんぱく質", "目安":f"{protein_target:.0f} g", "現在":f"{total('protein'):.0f} g", "差分":f"{protein_target-total('protein'):+.0f} g"},
        {"栄養素":"脂質", "目安":f"{fat_target:.0f} g", "現在":f"{total('fat'):.0f} g", "差分":f"{fat_target-total('fat'):+.0f} g"},
        {"栄養素":"炭水化物", "目安":f"{carb_target:.0f} g", "現在":f"{total('carbs'):.0f} g", "差分":f"{carb_target-total('carbs'):+.0f} g"},
    ])
    st.markdown("#### 今日の目安と差分")
    st.dataframe(targets, use_container_width=True, hide_index=True)
    st.caption("運動日は、有酸素の消費カロリーと筋トレ1件あたりの回復目安を加算しています。減量中でも、運動日の極端な食事不足は避ける設計です。")
    st.markdown("#### 食事")
    if meals:
    ...
    st.markdown("#### 次に食べるもの・栄養コーチ")
    st.caption("ビタミン・ミネラルなどは食事内容からの推定です。医療的な診断ではありません。")
    if st.button("今日の詳細な栄養アドバイスを作成", disabled=not meals):
        context = {"meals": meals, "strength": strength, "cardio": cardio,
                   "body": db.table("body_metrics").select("*").eq("logged_date",str(selected)).execute().data}
        try:
            st.session_state.nutrition_advice = nutrition_coach(context, goal, diet_notes)
        except Exception as exc:
            st.error(f"アドバイスを作成できませんでした: {exc}")
    if st.session_state.get("nutrition_advice"):
        st.info(st.session_state.nutrition_advice)
