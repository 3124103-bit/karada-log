...
.macro-card.below { background:rgba(142, 193, 236, .16); border-color:rgba(160, 209, 242, .45); }
.macro-card.over { background:rgba(242, 169, 178, .16); border-color:rgba(248, 185, 193, .45); }
...
with tabs[5]:
    ...
    if goal in ["アンチエイジング＋脂肪を落とす", "脂肪を落とす"]:
        ...
        rest_day_target = max(bmr * 1.15, bmr * 1.25 - 300)
        calorie_target = rest_day_target + cardio_burned + strength_bonus
    ...
    st.caption(f"目標：{goal}　/　{activity_note}　/　差分は「現在 − 目安」（マイナス＝不足、プラス＝目安超え）")
