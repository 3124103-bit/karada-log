...
with tabs[5]:
    ...
    st.caption(f"直近7日：食事ログ {week_days} 日。ビタミン・ミネラルは食事内容からの推定であり、医療的な診断ではありません。")
    c_week, c_month = st.columns(2)
    if c_week.button("7日間の傾向を分析", disabled=not week_meals):
        context = {"today_meals": meals, "week_meals": week_meals, "strength": strength, "cardio": cardio,
                   "body": db.table("body_metrics").select("*").eq("logged_date",str(selected)).execute().data,
                   "week_log_days": week_days}
        try:
            st.session_state.nutrition_advice_week = nutrition_coach(context, goal, diet_notes)
        except Exception as exc:
            st.error(f"アドバイスを作成できませんでした: {exc}")
    if c_month.button("30日間の傾向を分析", disabled=not month_meals_for_advice):
        context = {"today_meals": meals, "month_meals": month_meals_for_advice, "month_log_days": month_days_for_advice,
                   "body": body, "analysis_window": "直近30日"}
        try:
            st.session_state.nutrition_advice_month = nutrition_coach(context, goal, diet_notes)
        except Exception as exc:
            st.error(f"アドバイスを作成できませんでした: {exc}")
    if st.session_state.get("nutrition_advice_week"):
        st.markdown("**直近7日の傾向**")
        st.info(st.session_state.nutrition_advice_week)
    if st.session_state.get("nutrition_advice_month"):
        st.markdown(f"**直近30日の傾向（食事ログ {month_days_for_advice} 日）**")
        st.info(st.session_state.nutrition_advice_month)
